...
medicaid.share.plt <- final.data %>% group_by(year) %>% summarize(mean=mean(perc_medicaid)) %>% 
  ggplot(aes(x = year, y = mean)) +
  geom_line(linewidth = 1.2, color = "#0a0064") +
  geom_point(color = "#0f0064") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    x = "Year",
    y = "Fraction with Medicaid",
    title = "Share of Medicaide Insurance over Time"
  ) +
  theme_minimal()
...
save.image("submission2/results/homework5workspace.RData") 
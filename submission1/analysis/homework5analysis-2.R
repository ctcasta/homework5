...
#direct.share->direct.purchase
direct.purchase <- final.data %>%
  group_by(year) %>%
  summarize(
...
### plot
direct.purchase.plot <- ggplot(direct.purchase, aes(x = year, y = share_direct)) +
  geom_line(linewidth = 1.2, color = "steelblue") +
  geom_point(color = "steelblue") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Share of Adult Population with Direct Purchase of Health Insurance",
    x = "Year",
    y = "Share of Direct Purchase"
  ) +
  theme_minimal()

print(direct.purchase.plot)
...
### plot
medicaid.share.plot <- ggplot(medicaid.share, aes(x = year, y = share_medicaid)) +
  geom_line(linewidth = 1.2, color = "#1f066a") +
  geom_point(color = "#1f066a") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "Share of Adult Population with Medicaid",
    x = "Year",
    y = "Share with Medicaid"
  ) +
  theme_minimal()

print(medicaid.share.plot)
...
### join to main data 
final.data.expanded <- final.data %>%
  inner_join(medicaid.expanded, by = "State")

### calculate uninsured share by year and expansion group
uninsured.share <- final.data.expanded %>%
...
uninsured.plot <- ggplot(uninsured.share, aes(x = year, y = share_uninsured, color = expand_group)) +
  ...
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(
    title = "State's Uninsured Share by Medicaid Expansion Status",
    x = "Year",
    y = "Share Uninsured",
...
event.study14 <- ggplot(event.plot, aes(x = year, y = estimate)) +
      geom_line(color = "b4468c", linewidth = 1.2) +
      geom_point(size = 2, color = "b4468c) +
      geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
      geom_vline(xintercept = 2014, linetype = "dashed", color = "#5a3377") +
      geom_hline(yintercept = 0, linetype = "dotted") +
      labs(
        title = "Event Study of Medicaid Expansion on Uninsurance Rate",
        x = "Year",
        y = "Effect on Uninsurance",
      ) +
      theme_minimal()
print(event.study14)
...
event.study.all <- ggplot(event.plot.all, aes(x = event_time, y = estimate)) +
  ...
  geom_point(size = 2, color = "#b4468c") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "#5a3377") +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(
    title = "Generalized Event Study of Medicaid Expansion on Uninsurance",
    x = "Years Since Expansion",
    y = "Effect on Uninsurance Rate ",
  ) +
  theme_minimal()
...
rm(list = setdiff(ls(), c("direct.purchase.plot", "medicaid.share.plot", "uninsured.plot", "dd.table", "dd.model", "fe.model.tidy", "fe.model.all.tidy", "event.study14", "event.study.all")))
save.image("submission1/results/homework5_workspace.RData") 
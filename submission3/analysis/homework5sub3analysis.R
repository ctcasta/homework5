...
uninsured.plot <- ggplot(uninsured.share, aes(x = year, y = share_uninsured, linetype  = expand_group)) +
  ...
  geom_text(
    data = uninsured.share %>% 
      filter(year == 2018) %>%
      mutate(label = ifelse(expand_group == "Expanded in 2014", "Expansion", "Non-expansion"),
...
save.image("submission3/results/homework5sub3workspace.RData") 